...
svr.get("/admin/servers/add",(req,res)=>{
    res.render(`admin/servers/add`, { title: 'Add New Server' });
});
...
svr.post("/admin/servers/:sid/init",async(req,res)=>{
    ...
    res.json(await initServer(server,db.setting.get("monitor_tool_url")));
});
svr.post("/admin/servers/:sid/update",async(req,res)=>{
    var {sid}=req.params,
        server=db.servers.get(sid);
    res.json(await updateServer(server,db.setting.get("monitor_tool_url")));
});
svr.get("/admin/servers",(req,res)=>{
    res.render("admin_servers",{
...
svr.get("/get-monitor-tool",async(req,res)=>{
    var path=__dirname+'/monitor-tool';
